...
```{r}
plant_subset %>% 
  select(totmass:num.phylls) %>% 
  ggpairs() %>% 
  labs(caption = "Visual of ")
```
...
Using diagnostic tests and statistical assumptions the normality and homoscedastic assumptions appear to be violated. There are a few identified outliers
...
In order to meet the assumptions, I transformed the data to log data.

**i.**

```{r}
model_2_log <- lm(log(amass) ~ species, data = plant_subset)
model_3_log <- lm(log(feedlevel) ~ species, data = plant_subset)
